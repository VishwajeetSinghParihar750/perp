...
describe("OrderBook", () => {
  ...
  describe("createOrder - MARKET BUY", () => {
    it("should match a market buy against existing limit sell orders", () => {
      orderBook.createOrder("LIMIT", "SELL", "SOLUSD", 10, "seller1", 100, "ISOLATED", 20);
      const result = orderBook.createOrder("MARKET", "BUY", "SOLUSD", 5, "buyer1", 50, "ISOLATED");
    ...
    it("should partially fill market buy if not enough liquidity", () => {
      orderBook.createOrder("LIMIT", "SELL", "SOLUSD", 3, "seller1", 100, "ISOLATED", 20);
      const result = orderBook.createOrder("MARKET", "BUY", "SOLUSD", 10, "buyer1", 50, "ISOLATED");
      expect(result.totalFilledQuantity).toBe(3);
      expect(result.fills).toHaveLength(1);
    });

    it("should match across multiple price levels on market buy", () => {
      orderBook.createOrder("LIMIT", "SELL", "SOLUSD", 5, "seller1", 100, "ISOLATED", 20);
      orderBook.createOrder("LIMIT", "SELL", "SOLUSD", 5, "seller2", 100, "ISOLATED", 21);
      const result = orderBook.createOrder("MARKET", "BUY", "SOLUSD", 8, "buyer1", 50, "ISOLATED");
  ...
  describe("createOrder - MARKET SELL", () => {
    it("should match a market sell against existing limit buy orders", () => {
      orderBook.createOrder("LIMIT", "BUY", "SOLUSD", 10, "buyer1", 100, "ISOLATED", 20);
      const result = orderBook.createOrder("MARKET", "SELL", "SOLUSD", 5, "seller1", 50, "ISOLATED");
      expect(result.totalFilledQuantity).toBe(5);
      expect(result.fills).toHaveLength(1);
      expect(result.fills[0].price).toBe(20);
    });
  ...
  describe("createOrder - LIMIT BUY", () => {
    it("should place a limit buy order in the orderbook when no match", () => {
      const result = orderBook.createOrder("LIMIT", "BUY", "SOLUSD", 5, "buyer1", 100, "ISOLATED", 20);
    ...
    it("should match limit buy against existing sell orders at or below price", () => {
      orderBook.createOrder("LIMIT", "SELL", "SOLUSD", 5, "seller1", 100, "ISOLATED", 18);
      const result = orderBook.createOrder("LIMIT", "BUY", "SOLUSD", 5, "buyer1", 100, "ISOLATED", 20);
      expect(result.totalFilledQuantity).toBe(5);
      expect(result.fills).toHaveLength(1);
    });
    ...
    it("should partially fill limit buy and place remainder in orderbook", () => {
      orderBook.createOrder("LIMIT", "SELL", "SOLUSD", 3, "seller1", 100, "ISOLATED", 18);
      const result = orderBook.createOrder("LIMIT", "BUY", "SOLUSD", 8, "buyer1", 100, "ISOLATED", 20);
      expect(result.totalFilledQuantity).toBe(3);
      expect(result.fills).toHaveLength(1);
  ...
  describe("cancelOrder", () => {
    it("should cancel an open order and return its details", () => {
      orderBook.createOrder("LIMIT", "BUY", "SOLUSD", 5, "buyer1", 100, "ISOLATED", 20);
  ...
  describe("getDepth", () => {
    ...
    it("should respect count param", () => {
      for (let i = 0; i < 10; i++) {
        orderBook.createOrder("LIMIT", "BUY", "SOLUSD", 1, "buyer1", 100, "ISOLATED", 20 + i);
      }
  ...
  describe("getSnapshot / loadSnapshot", () => {
    it("should roundtrip snapshot correctly", () => {
      orderBook.createOrder("LIMIT", "BUY", "SOLUSD", 5, "buyer1", 100, "ISOLATED", 20);
      orderBook.createOrder("LIMIT", "SELL", "SOLUSD", 5, "seller1", 100, "ISOLATED", 25);
